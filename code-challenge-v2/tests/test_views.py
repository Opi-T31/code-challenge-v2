...
def test_map_data_view():
    ...
    # Create test permits for Beverly
    ...
    # Create test permits for Lincoln Park
    RestaurantPermit.objects.create(
    ...
    # Query the map-data endpoint with year filter
    client = APIClient()
    response = client.get(reverse("map_data"), {"year": 2021})

    # Verify the API returned a successful response
    assert response.status_code == 200

    # Convert JSON response into Python data
    data = response.json()

    # Convert list of areas into a dictionary keyed by name
    # This makes it easier to check values
    results = {area["name"]: area for area in data}

    # Assert correct permit counts
